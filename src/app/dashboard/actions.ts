...
export async function seedDemoEngagements() {
  ...
  const { error: e1Err } = await supabase.from('engagements').insert({
    ...
    template_id: null,
    mode: 'custom_outcome',
    title: 'End Client: Hertz — Car Rental App Intelligence Layer',
    status: 'active',
    scope_summary: 'Building an AI intelligence layer for Hertz\'s car rental application using a leading LLM API. The system handles natural language fleet availability queries, intelligent price optimisation recommendations, predictive maintenance scheduling, and automated customer service responses — all integrated with Hertz\'s existing reservation system and fleet management platform.',
  ...
  const { error: m1Err } = await supabase.from('milestones').insert([
    {
      ...
      description: 'Stakeholder alignment with Hertz engineering leads, API surface mapping, LLM integration strategy, and system architecture sign-off.',
      status: 'completed',
      deliverables: [
        { name: 'System architecture diagram', status: 'done' },
        { name: 'LLM API integration strategy', status: 'done' },
    ...
    {
      engagement_id: e1Id,
      title: 'LLM API Integration & PoC',
      description: 'Live LLM API integration, prompt engineering for rental domain, PoC demonstrating natural language fleet queries with real Hertz test data.',
      status: 'completed',
      deliverables: [
        { name: 'LLM API wrapper service', status: 'done' },
    ...
    },
    {
      ...
      description: 'End-to-end QA pass, load testing under realistic Hertz traffic volumes, accuracy benchmarking for LLM responses, and security review.',
      status: 'upcoming',
      deliverables: [
        { name: 'E2E test suite (150+ scenarios)', status: 'pending' },
        { name: 'Load test report (10K req/min)', status: 'pending' },
        { name: 'LLM response accuracy report (target: 94%+)', status: 'pending' },
  ...
  await supabase.from('messages').insert([
    ...
    { engagement_id: e1Id, sender_name: 'Carlos Mendez (PM)', sender_role: 'pm', content: 'Hi team — kickoff call with Hertz went well. Their engineering leads are aligned on the LLM-first approach. Architecture doc is in Notion. Marcus and Priya, please review the Hertz API docs I shared — we start integration Monday.', is_system_message: false, created_at: '2026-02-03T14:30:00+00' },
    { engagement_id: e1Id, sender_name: 'Marcus Chen', sender_role: 'engineer', content: 'Hertz API docs reviewed. Their fleet availability endpoint is REST, well-documented, 200ms P95. I\'m thinking we wrap it with a semantic caching layer so the model isn\'t firing live API calls for every query — should cut latency by ~60%. Will have a design proposal by EOD.', is_system_message: false, created_at: '2026-02-05T10:15:00+00' },
    ...
    { engagement_id: e1Id, sender_name: 'System', sender_role: 'system', content: 'Milestone 2 (LLM API Integration & PoC) marked as completed.', is_system_message: true, created_at: '2026-03-06T17:30:00+00' },
  ...
  await supabase.from('milestones').insert([
    {
      ...
      description: 'Audit of existing test infrastructure across all repos, framework selection (pytest + Playwright + Jest), LLM integration strategy for AI-powered test generation.',
      status: 'completed',
      deliverables: [
        { name: 'Codebase audit across 12 repos', status: 'done' },
        { name: 'Framework selection document', status: 'done' },
        { name: 'AI test-gen integration design', status: 'done' },
    ...
    },
    {
      ...
      description: 'AI-powered automatic test generation for new code, PR-level test suggestions, and regression test auto-creation from bug reports. Delivered for client review.',
      status: 'in_review',
      deliverables: [
        { name: 'AI test-gen service (PR hook)', status: 'done' },
  ...
  await supabase.from('messages').insert([
    ...
    { engagement_id: e2Id, sender_name: 'Nadia Torres', sender_role: 'engineer', content: 'The AI test-gen service is deployed. It\'s hooking into PR webhooks and suggesting tests for any new function or module. In the past 3 days it\'s generated 47 test suggestions across 8 open PRs — engineers are accepting about 60% of them as-is, rest with minor edits. Ready for your review, Carlos.', is_system_message: false, created_at: '2026-04-01T14:00:00+00' },
    { engagement_id: e2Id, sender_name: 'Carlos Mendez (PM)', sender_role: 'pm', content: 'Milestone 3 is in review — please take a look at the AI test-gen output and the test quality scores in the dashboard. Overall project health is excellent. We\'re on track for full rollout and final handoff by April 18th.', is_system_message: false, created_at: '2026-04-03T10:00:00+00' },
  ])
